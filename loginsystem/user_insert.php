    ...
    if(isset($_POST['set'])) {
		...
		if(isset($_POST['check'])){
				...
				if(!mysqli_stmt_prepare($result, $sql)){
						...
						if($row = mysqli_fetch_assoc($resultl)){
								if($row['Supicious'] == 0){
										header("location: AddCard.php?error=No_Need");
										exit();
								}
								else{
										$supicious = 0;
										$sql = "UPDATE users SET Supicious=? WHERE CardID_select=?";
										$result = mysqli_stmt_init($conn);
										if(!mysqli_stmt_prepare($result, $sql)){
												header("location: AddCard.php?error=SQL_Error");
												exit();
												...
												mysqli_stmt_bind_param($result, "ii", $supicious, $Card_sel);
												mysqli_stmt_execute($result);
												header("location: AddCard.php?success=Checked");
												exit();
										}
								}
						}
